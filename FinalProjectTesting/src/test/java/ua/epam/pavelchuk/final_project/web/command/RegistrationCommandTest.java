...
public class RegistrationCommandTest {
	...
	@Test
	public void doPost() throws AppException, IOException, ServletException, DBException {
		...
		try (MockedStatic<UserDAO> userDAOMockedStatic = mockStatic(UserDAO.class);
				MockedStatic<UserValidator> userValidationMockedStatic = mockStatic(UserValidator.class)) {
			when(UserDAO.getInstance()).thenReturn(userDAO);
			when(UserValidator.validationLogin(anyString())).thenReturn(true);
			when(UserValidator.validate(any(HttpServletRequest.class), anyString(), anyString(), anyString(), any(User.class),anyString(),anyString())).thenReturn(true);
